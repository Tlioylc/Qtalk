...
public class SessionDao {
	...
	protected SessionDao(Context context) {
		db = DBHelper.getInstance(context).getWritableDatabase();
	}

	protected SessionDao(Context context, int version) {
		db = DBHelper.getInstance(context).getWritableDatabase();
	}
	...
	public long insertSession(Session session) {
		if(session.getToUser().equals(session.getFromUser())){
			return 0;
		}
		ContentValues values = new ContentValues();
		values.put(DBcolumns.SESSION_FROM, session.getFromUser());
		values.put(DBcolumns.SESSION_TIME, session.getDate());
	...
	public long updateSession(Session session) {
		...
		values.put(DBcolumns.SESSION_TIME, session.getDate());
		values.put(DBcolumns.SESSION_CONTENT, session.getContent());
		long row = db.update(DBcolumns.TABLE_SESSION, values, DBcolumns.SESSION_FROM + " = ? and " + DBcolumns.SESSION_TO + " = ?", new String[] { session.getFromUser(),session.getToUser() });
		return row;
	}
	...
	public long deleteSession(Session session) {
		long row = db.delete(DBcolumns.TABLE_SESSION, DBcolumns.SESSION_FROM+"=? and "+DBcolumns.SESSION_TO+"=?", new String[] { session.getFromUser(),session.getToUser() });
		return row;
	}
