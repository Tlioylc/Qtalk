...
public class QMMContactsManager {
    ...
    public void deleteContanct(final String userName){
        QtalkClient.getInstance().getQmChatMessageManager().deleteConversationAllMessage(QtalkClient.getInstance().getUser().split("@")[0],userName);
        QtalkClient.getInstance().execute(new Runnable() {
            @Override
            public void run() {
                removeUser(userName);
    ...
    public boolean agreeApply(String userName){
        ...
        presenceRes.setTo(userName+"@"+QtalkClient.getInstance().getServiceName());
        LogUtil.e(userName+"@"+QtalkClient.getInstance().getServiceName());
        try {
            QtalkClient.getInstance().sendStanza(presenceRes);
            addUser(userName);
            return true;
    ...
    {
        try {
            roster.createEntry(userName+"@"+qtalkClient.getServiceName(), userName, null);
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    {
        try {
            roster.createEntry(userName+"@"+qtalkClient.getServiceName(), userName,new String[]{ groupName});
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
